...
//        let requestLocationTapped: AnyPublisher<Void, Never>
    ...
    struct Output {
        /// test
//        let oneTime: AnyPublisher<[Place], Never>
        /// 위치 퍼블리셔 노출
    ...
    func transform(input: Input) -> Output {
        // viewDidLoad 시 딱 한번 권한 + 위치 요청 + 장소 추천 리스트 (업데이트 X)
//        var oneTime: [Place] = []
        input.viewDidLoad
            .sink { [weak self] in
...
//                    }.store(in: &cancellable)
        ...
        // center: 지도 중심이나 현재 위치를 흘려주는 퍼블리셔
        let center: AnyPublisher<CLLocationCoordinate2D, Never> = locationUseCase.locationPublisher.map { $0.coordinate }.eraseToAnyPublisher()
        ...
        let placesRaw: AnyPublisher<[Place], Never> = searchQuery
            ...
            .combineLatest(center)                         // -> (query: String, center: CLLocationCoordinate2D)
            // 최신 요청만 유지
            .map { [weak self] (query, center) -> AnyPublisher<[Place], Never> in
                guard let self = self else { return Just([]).eraseToAnyPublisher() }
                return self.searchUseCase
                    .search(keyword: query, center: center) // ✅ 변경 포인트
                    ...
                    return URLSession.shared.dataTaskPublisher(for: url)
                        ...
                        .flatMap { image in
                            ...
                            self.imageUseCase.upload(
                                ...
                                fileName: "\(place.placeId).jpg"        // 고유 파일명
                            )
                        }
                        .catch { error in
