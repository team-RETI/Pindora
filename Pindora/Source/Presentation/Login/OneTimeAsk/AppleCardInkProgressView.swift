...
//    
//    enum InkMode {
//        case fixedCenter      // ✅ 첫 탭으로 중심 고정, 이후 반경만 증가(권장)
//        case accumulateUnion  // 탭할 때마다 원을 합집합으로 누적
//    }
//    
//    // MARK: - Public
...
//    var verticalBias: CGFloat = 0.35 // 0~1, 중앙으로 섞는 비율
//    
//    // MARK: - Layers & UI
...
//    
//    // MARK: - State
//    private var unionPath: CGMutablePath?      // accumulateUnion용
//    
//    // MARK: - Init
...
//        
//        // 고정중심 모드: 리레이아웃 시 고정 Blob 위치/크기 유지
...
//        case .fixedCenter:
//            if lockedCenter == nil {
//                lockedCenter = CGPoint(x: bounds.midX, y: bounds.midY) // 중앙 고정(원하면 다른 기준점도 OK)
//            }
...
//        g.type = .radial
//        // 마스크라서 '흰색 alpha'만 중요. 중앙 1 → 가장자리 0
//        g.colors = [
//            UIColor.white.withAlphaComponent(1).cgColor,
//            UIColor.white.withAlphaComponent(1).cgColor,
//            UIColor.white.withAlphaComponent(0).cgColor
//        ]
//        g.locations = [0.0, 0.65, 1.0] as [NSNumber] // ⬅️ Feather 폭 조절
//        g.startPoint = CGPoint(x: 0.5, y: 0.5)       // 중심
//        g.endPoint   = CGPoint(x: 1.0, y: 1.0)       // 반경
//        let d = radius * 2
...
//        }
//        
//        let blob = fixedCenterBlob ?? makeFeatherBlob(center: c, radius: max(12, targetR * 0.2))
//        let currentScale: CGFloat = (blob.presentation() ?? blob).value(forKeyPath: "transform.scale.x") as? CGFloat ?? 1
//        let targetScale = (targetR * 2) / blob.bounds.width
//        
...
//        let anim = CABasicAnimation(keyPath: "transform.scale")
//        anim.fromValue = 0.01
//        anim.toValue   = (targetR * 2) / blob.bounds.width
//        anim.duration  = 0.35
//        anim.timingFunction = CAMediaTimingFunction(name: .easeInEaseOut)
//        blob.add(anim, forKey: "scale")
//        blob.transform = CATransform3DMakeScale((targetR * 2) / blob.bounds.width,
//                                                (targetR * 2) / blob.bounds.width,
//                                                1)
//    }
//    
//    // MARK: - Styling / Utils
